...
public class GoNativeWindowManager {
    ...
    private final List<MaxWindowsListener> maxWindowsListeners;
    ...
    public GoNativeWindowManager() {
        ...
        maxWindowsListeners = new ArrayList<>();
    }
    ...
    public void notifyMaxWindowsReached(String excessActivityId) {
        if (maxWindowsListeners.isEmpty()) return;

        Iterator<MaxWindowsListener> iterator = maxWindowsListeners.iterator();
        while(iterator.hasNext()) {
            MaxWindowsListener listener = iterator.next();
            if (listener.onMaxWindowsReached(excessActivityId)){
                iterator.remove();
            }
        }
    }

    public void addMaxWindowsListener(MaxWindowsListener listener) {
        this.maxWindowsListeners.add(listener);
    }
    ...
    public void setCurrentActiveWindowId(String activityId) {
        this.currentActiveWindowId = activityId;
    }
    ...
    interface MaxWindowsListener {
        boolean onMaxWindowsReached(String excessWindowId);
    }
}
