...
import co.median.android.ads.AdsBridge;
...
public class WebViewSetup {
    ...
    public static void setupWebviewForActivity(GoNativeWebviewInterface webview, MainActivity activity) {
        ...
        // NativeAds bridge (AdMob / AppLovin flavor-selected at build time).
        wv.removeJavascriptInterface("__AdsBridge");
        wv.addJavascriptInterface(
                new AdsBridge(() -> activity, () -> (wv instanceof android.webkit.WebView) ? (android.webkit.WebView) wv : null),
                "__AdsBridge");
        wv.evaluateJavascript(AdsBridge.INJECTED_JS, null);

        ((GoNativeApplication) activity.getApplication()).mBridge.onWebviewSetUp(activity, wv);

        if (activity.isFromWindowOpenRequest()) {
            ...
            if (resultMsg != null) {
                ...
                try {
                    ...
                    GNLog.getInstance().logError(TAG, ex.getMessage(), ex);
                }
            }
        }
    }
    ...
    public static void setupWebview(GoNativeWebviewInterface webview, Context context) {
        ...
        if (TextUtils.isEmpty(userAgent)) {
            ...
            } else if (!TextUtils.isEmpty(appConfig.userAgentAdd)) {
                // create a userAgent with the device userAgent plus additional string
                userAgent = webview.getDefaultUserAgent() + " " + appConfig.userAgentAdd;
            } else {
                userAgent = webview.getDefaultUserAgent();
            }
        }
        ...
        if (appConfig.disableOverScrollAnimation) {
