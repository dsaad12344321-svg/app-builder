...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // Bootstrap ads network (AdMob or AppLovin, selected via SWIFT_ACTIVE_COMPILATION_CONDITIONS).
    NSString *cfgPath = [[NSBundle mainBundle] pathForResource:@"appConfig" ofType:@"json"];
    NSData *cfgData = cfgPath ? [NSData dataWithContentsOfFile:cfgPath] : nil;
    NSDictionary *cfgDict = nil;
    if (cfgData) {
        cfgDict = [NSJSONSerialization JSONObjectWithData:cfgData options:0 error:nil];
    }
    [[AdsHub shared] bootstrapWithConfigDict:cfgDict ?: @{}];
...
- (void)applicationWillEnterForeground:(UIApplication *)application
{
    dispatch_async(dispatch_get_main_queue(), ^{
        UIViewController *rvc = self.window.rootViewController;
        if ([rvc isKindOfClass:[LEANRootViewController class]]) {
            if ([GoNativeAppConfig sharedAppConfig].injectMedianJS) {
                [(LEANRootViewController *)rvc runJavascriptWithCallback:@[@"median_app_resumed", @"gonative_app_resumed"] data:nil];
            } else {
                [(LEANRootViewController *)rvc runJavascriptWithCallback:@"_median_app_resumed" data:nil];
            }
        }
    });
