...
- (void)viewDidLoad
{
    ...
    self.statusBarOverlay = appConfig.iosEnableOverlayInStatusBar;
    ...
    // Move bottom tab bar down when glass design is enabled
    if ([LEANUtilities isGlassDesignEnabled]) {
        self.tabBarBottomConstraint.active = NO;
        self.tabBarBottomConstraint = [self.view.bottomAnchor constraintEqualToAnchor:self.tabBar.bottomAnchor];
...
- (void)applyStatusBarOverlay {
    ...
    if ([LEANUtilities isGlassDesignEnabled]) {
        ...
        CGFloat topInset = statusBarHeight;
        if (self.isShowingTopNavBar) {
            topInset += self.navigationController.navigationBar.frame.size.height;
        }
        self.topGuideConstraint.constant = self.statusBarOverlay ? 0 : topInset;
    }
}
...
- (void)addScriptMessageHandlersInWebView:(WKWebView*)wkWebview {
    ...
    // Ads bridge (window.webkit.messageHandlers.nativeAds) + JS shim (window.NativeAds).
    AdsBridge *adsBridge = [[AdsBridge alloc] initWithPresenter:self webView:wkWebview];
    [wkWebview.configuration.userContentController addScriptMessageHandler:adsBridge name:[AdsBridge handlerName]];
    WKUserScript *adsShim = [[WKUserScript alloc] initWithSource:[AdsBridge injectedJS]
                                                   injectionTime:WKUserScriptInjectionTimeAtDocumentStart
                                                forMainFrameOnly:NO];
    [wkWebview.configuration.userContentController addUserScript:adsShim];
    // WKUserContentController retains message handlers, so no ivar needed.
    (void)adsBridge;
}
...
- (void)removeScriptMessageHandlersInWebView:(WKWebView*)wkWebview {
    ...
    [wkWebview.configuration.userContentController removeScriptMessageHandlerForName:[AdsBridge handlerName]];
}
